...
class MenuRepository:
    def __init__(self, runner: BigQueryRunner, settings: Settings) -> None:
        ...
        self._description_column = quote_column(settings.menu_description_column)
        self._price_column = quote_column(settings.menu_price_column)
        self._currency_column = quote_column(settings.menu_currency_column)
        self._is_available_column = quote_column(settings.menu_is_available_column)
    ...
    def list_menu_items(self, params: MenuQueryParams) -> list[dict]:
        ...
        query = f"""
            SELECT
                ...
                CAST({self._description_column} AS STRING) AS description,
                COALESCE(SAFE_CAST({self._price_column} AS FLOAT64), 0.0) AS price,
                COALESCE(CAST({self._currency_column} AS STRING), 'USD') AS currency,
                COALESCE(CAST({self._is_available_column} AS BOOL), TRUE) AS is_available
            FROM {self._table}
            {where_sql}
